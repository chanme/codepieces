/**
 * codepieces from vscode on the diff algorithm.
 * 1. implement the O(ND) myers algorithn for edit distance
 * 2. optimization on time (forward and reverse diretion)
 * 3. optimization on space (when space is not enough using the possible recurssive point)
 * 4. a lot of messy details in handling the diagonal indexs
 */
...
export class LcsDiff {
	...
	private ComputeDiffRecursive(originalStart: number, originalEnd: number, modifiedStart: number, modifiedEnd: number, quitEarlyArr: boolean[]): DiffChange[] {
		...
		// 下面是处理全是插入或者删除的情况
		// 1. originStart > originalEnd 说明修改的结果是从原序列在尾部追加   abcdef => abcdefghi
		// 2. modifiedStart > modifiedEnd 说明修改的结果是将原序列的头部删除  abcdef => def
		// 3. 或者是一模一样
		if (originalStart > originalEnd || modifiedStart > modifiedEnd) {
			let changes: DiffChange[];

			// 1. 尾部追加 abcdef|  abcdef|ghi
			if (modifiedStart <= modifiedEnd) {
				Debug.Assert(originalStart === originalEnd + 1, 'originalStart should only be one more than originalEnd');

				// All insertions
				changes = [
					// 这个时候originalStart == originalEnd，所以是将尾部的origin
					new DiffChange(originalStart, 0, modifiedStart, modifiedEnd - modifiedStart + 1)
				];

				// 2. 原序列头部完全删除  abc|def |def
		...
		if (result !== null) {
			...
			// Second Half:  (midOriginal + 1, midModified + 1) to (originalEnd, modifiedEnd)
	...
	private ComputeRecursionPoint(originalStart: number, originalEnd: number, modifiedStart: number, modifiedEnd: number, midOriginalArr: number[], midModifiedArr: number[], quitEarlyArr: boolean[]) {
		...
		// originEnd-originalStart modifiedEnd - modifiedStart 分别等于传统的n,m
		...
		// 考察一下整个矩阵如下图, a=>(os,ms) d=>(oe,me) c=>(os,me) b=>(oe,ms) 所以对角线的下标的范围是c,b的两点
		// 矩阵对角线的唯一标识可以采用(x,y)的差值，但负数是我们不喜欢的, 我们如果强制以(os,me)作为0,那么对应的(os,me)就是me-ms
		// 所以这里的diagonalForwardBase正是经过(os,ms)的对角线索引
		// ---------os-----------oe----
		// ---ms----axxxxxxxxxxxxxb----
		// ---------x-------------x----
		// ---------x-------------x----
		...
		// diagonalForwardOffset则是实际上的数值下标
		...
		for (numDifferences = 1; numDifferences <= (maxDifferences / 2) + 1; numDifferences++) {
			...
			// 这里实际上是获得实际会变化的的对角线区间 ClipDiagonalBound做的事情是根据奇偶行找出超出边界的开始和结束边界（因为这里的更新是奇数更新偶数）
			...
			for (diagonal = diagonalForwardStart; diagonal <= diagonalForwardEnd; diagonal += 2) {
				...
				// 当前对角线处于左边界，或者说右侧对角线的值扩展的更远，则选取右侧的
				if (diagonal === diagonalForwardStart || (diagonal < diagonalForwardEnd && forwardPoints[diagonal - 1] < forwardPoints[diagonal + 1])) {
					// 这里是因为目标点向下移一格扩展左侧，下标没变
					originalIndex = forwardPoints[diagonal + 1];
				} else {
					// 这里是目标点向右移一格扩展右侧，下标+1
					originalIndex = forwardPoints[diagonal - 1] + 1;
				}
				// 相应的去算修改后的坐标，坐标转换一下
	...
	private ClipDiagonalBound(diagonal: number, numDifferences: number, diagonalBaseIndex: number, numDiagonals: number): number {
		...
		// diagonalsBelow: The number of diagonals below the reference diagonal 处在主对角线之下的对角线数
		// diagonalsAbove: The number of diagonals above the reference diagonal 处在主对角线之上的对角线数
